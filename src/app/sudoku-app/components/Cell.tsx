...
import { EMPTY } from "@/core/sudoku-app/constants/sudoku-constants";
...
const Container = styled(GridItem, { shouldForwardProp: (propName) => propName !== 'theme' && !propName.startsWith("$")})<{
    $selected: boolean;
}>`
...
const SelectedCell = styled("div", { shouldForwardProp: (propName) => propName !== 'theme' && !propName.startsWith("$")})<{
    $selected: boolean;
...
const MutableCell = styled("div", { shouldForwardProp: (propName) => propName !== 'theme' && !propName.startsWith("$")})<{
    $wrong: boolean;
}>`
